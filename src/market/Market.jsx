...
const Market = ({ data, game, config, title }) => {
  ...
  if (data.config.stock.display.par && data.display.par) {
    ...
    const x = data.display.par.x * data.config.stock.cell.width;
    const y = data.display.par.y * data.config.stock.cell.height;
    par = (
  ...
  if (data.type === "2D") {
    ...
    const x = game.stock.display.legend.x * config.stock.cell.width;
    const y = game.stock.display.legend.y * config.stock.cell.height;
    ...
    const legend = (game.stock && game.stock.legend) || [];
    let left = 0;
...
return (
            <g
              key={`pool-note-${i}`}
              transform={`translate(${current} ${Number(data.height) + (data.stock.title === false ? 25 : 75)})`}
            >
              <Legend {...legend}/>
            </g>
          );
